...
export default {
    init: function (ev, objects, name) {
        ...
        THREE.ShaderChunk[name + "_pars"] = `
            ...
            void ${name}_apply(inout vec4 fragColor, in vec2 uv) {
                fragColor.rgb += texture2D(${name}_texture, uv).rgb;
            }
        `;
        ...
        controlUniforms.strength = scene.userData[name + "_strength"] = postUniforms.strength;
        controlUniforms.radius = scene.userData[name + "_radius"] = postUniforms.radius;
        if (preUniforms.threshold) controlUniforms.threshold = scene.userData[name + "_threshold"] = preUniforms.threshold;
        scene.userData[name + "_texture"] = { value: ping[0].texture };
        ...
        var postPass = getPass(`
            ...
            void main() {
                gl_FragColor = strength * ( lerpBloomFactor(1., 0.2) *  texture2D(blurTexture1, vUv) + \
                                                lerpBloomFactor(0.2, 0.8) *  texture2D(blurTexture2, vUv) + \
                                                lerpBloomFactor(0.2, 1.) *  texture2D(blurTexture3, vUv) );\
            }
        `, postUniforms);

        scene.userData[name + "_internal"] = {prePass, blurPasses, postPass};
        ...
        controlUniforms[""] = function(){
            ...
            delete scene.userData[name + "_internal"];
            
            for( var k in ev.userData[name]) {
